...
const MovieItem = memo(({ movie, onView, onEdit, onDelete }: {
  movie: Movie;
  ...
  return (
    <div style={{ border: '1px solid #ccc', padding: '16px', margin: '8px 0' }}>
      ...
      {movie.poster_url && (
        <p><strong>Poster:</strong> <a href={movie.poster_url} target="_blank" rel="noopener noreferrer">Ver Poster</a></p>
      )}
      <p><strong>Criado em:</strong> {new Date(movie.created_at).toLocaleDateString('pt-BR')}</p>
      
      <div style={{ marginTop: '12px' }}>
        <button onClick={() => onView(movie.id)} style={{ marginRight: '8px' }}>
          Visualizar
        </button>
        <button onClick={() => onEdit(movie.id)} style={{ marginRight: '8px' }}>
          Editar
        </button>
...
const MovieFilters = memo(({ onFilter }: { onFilter: (filters: FilterFormData) => void }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} style={{ marginBottom: '24px', padding: '16px', border: '1px solid #ddd' }}>
      <h3>Filtros</h3>
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        />
        {errors.search && <span style={{ color: 'red' }}>{errors.search.message}</span>}
      </div>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
        <div style={{ flex: 1 }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '8px', marginTop: '4px' }}
          />
          {errors.yearFrom && <span style={{ color: 'red' }}>{errors.yearFrom.message}</span>}
        </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '8px', marginTop: '4px' }}
          />
          {errors.yearTo && <span style={{ color: 'red' }}>{errors.yearTo.message}</span>}
        </div>
      </div>
...
const ListMovies: React.FC = () => {
  ...
  // Função de filtro
  const handleFilter = useCallback((filters: FilterFormData) => {
    let filtered = [...movies];
  ...
  const handleViewMovie = useCallback((id: number) => {
    navigate(`/movies/${id}`);
  }, [navigate]);
    ...
    <div style={{ padding: '20px' }}>
      ...
      {error && (
        <div style={{ backgroundColor: '#f8d7da', color: '#721c24', padding: '12px', marginBottom: '16px', border: '1px solid #f5c6cb' }}>
          ...
          <button 
            ...
            style={{ marginLeft: '12px', background: 'none', border: 'none', color: '#721c24', cursor: 'pointer' }}
          >
            ✕
          </button>
        </div>
      ...
      <div style={{ marginBottom: '16px' }}>
        <p>
          ...
          {filteredMovies.length !== movies.length && ` de ${movies.length} total`}
        </p>
      </div>
      ...
      {filteredMovies.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px' }}>
          ...
          {movies.length === 0 ? (
            <button onClick={handleCreateMovie} style={{ padding: '8px 16px', backgroundColor: '#007bff', color: 'white' }}>
